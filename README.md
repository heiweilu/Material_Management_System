...
| 模块 | 功能 |
|------|------|
| 📋 物料管理 | 物料 CRUD、搜索筛选、分类管理（树形结构）、低库存高亮 |
| 📥 入库管理 | 新建入库单、自动增加库存、历史记录查看/删除（回退库存） |
| 📤 出库管理 | 新建出库单、库存充足性校验、历史记录查看/删除（回退库存） |
| 📊 库存中心 | 库存总览（统计卡片）、低库存预警列表、分类库存饼图、出入库趋势折线图 |
| 🏢 供应商管理 | 供应商 CRUD、搜索 |
| 🔄 数据管理 | Excel/CSV 导入导出、导入模板下载、SQLite 数据库备份/恢复 |
| ⚙️ 系统设置 | 明暗主题切换、默认参数配置、日志级别设置 |
...
- **图表**: matplotlib（嵌入 Qt 的 FigureCanvas）
...
| `supplier` | 供应商信息 |
| `inbound_order` | 入库单头 |
| `inbound_detail` | 入库单明细 |
| `outbound_order` | 出库单头 |
| `outbound_detail` | 出库单明细 |
| `operation_log` | 操作日志 |
