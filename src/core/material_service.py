...
def create_material(data: dict) -> Material:
    ...
    try:
        ...
        _log(session, "create", "material", m.id,
             f"新增物料: {m.material_code} - {m.material_name}")
        session.commit()
        session.refresh(m)
        logger.info("新增物料: %s", m.material_code)
        return m
    except Exception:
        session.rollback()
        raise
...
def update_material(material_id: int, data: dict) -> Material | None:
    ...
    try:
        ...
        _log(session, "update", "material", m.id,
             f"编辑物料: {m.material_code} - {m.material_name}")
        session.commit()
        session.refresh(m)
        logger.info("更新物料: %s", m.material_code)
        return m
    except Exception:
        session.rollback()
        raise
...
def delete_material(material_id: int) -> bool:
    ...
    try:
        ...
        if m is None:
            return False
        desc = f"删除物料: {m.material_code} - {m.material_name}"
        session.delete(m)
